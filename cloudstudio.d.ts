...
declare module "cloudstudio-extension" {
  ...
  export namespace editor {
     ...
     * ```
     */
    export function getActiveEditor(): IActiveEditor;
     ...
     *   loader: () => monaco.Promise.wrap({ language: myLanguage, conf: myLanguageConf }) // 语法高亮配置
     * };
    ...
    interface IEditorViewConfig {
      contentTypes?: Array<string>,
      extensions?: Array<string>,
    }
    /**
     * 注册自定义编辑器
     * @param editor 需要注册的编辑器组件
     * @param config 配置对象，设置拦截条件。contentTypes 是文件类型集合，比如 ['image/png', 'image/webp']，extensions 是文件后缀集合，比如 ['js', 'jsx', 'vue']
     */
    export function registerEditorView(
      editor: any,
      config: IEditorViewConfig,
    ): any
    ...
    type IEditorDidMountHandler = (editor: monaco.editor.IStandaloneCodeEditor) => void;
    /**
     * 注册编辑器实例被创建时的回调函数
     *
    ...
    interface IEditorActiveArgs {
      uri: string;
      editor: monaco.editor.IStandaloneCodeEditor;
    }
    type IEditorActiveHandler = (arg: IEditorActiveArgs) => void;
    ...
    type SnippetsItemKindType = SnippetsItemKind
    /**
     * 代码片段实体
     */
    interface ISnippets {
  ...
  export namespace modal {
     ...
     *
     * 第三个参数 command 可以调用 Command 模块中的 executeCommand 方法来弹出模态框
     */
  ...
  export namespace git {
     ...
     * @return {Promise<Array<IGitLog> | null>} gitLog 该文件 Git 提交日志
     */
    export function gitFileLogs(
      config: IFileLogsConfig
    ): Promise<Array<IGitLog> | null>;
     ...
     * @return {Promise<Array<IGitRef> | null>} gitRef 项目 GitRefs
     */
    export function gitRefs(): Promise<Array<IGitRef> | null>;
     ...
     * @return {Promise<IFileContent>} fileContent 文件内容
     *
     ...
     * @return {Promise<IFileDiff>} 文件原始 diff 信息
     *
     ...
     * @return {Promise<Array<IGitBlame>>} gitBlame
     *
     * 查看 https://git-scm.com/docs/git-blame 了解 GitBlame 具体细节
     */
    export function gitBlame(filePath: string): Promise<Array<IGitBlame>>;
  }
  ...
  export namespace terminal {
    ...
    type ITerminalOpenFn = (terminal: ITerminal) => void;
    /**
     * 注册终端打开事件回调函数
     *
     ...
     * @return {Promise<ITerminal>} terminal 新建的终端
     * ```javascript
     * Terminal.createTerminal('my-terminal', '/home/coding/workspace/java')
     * .then(term => term.sendText('npm install'))
     * ```
     */
    export function createTerminal(id: string, shellPath?: string): Promise<ITerminal>;
  }
  /**
   * 文件操作 API
   */
  export namespace file {
     ...
     *
     * @param {string} filePath 文件路径
     * @return {Promise<IFileNode | null>} 文件 FileNode 实例
     */
    ...
    export function findLanguageByextension(ext: string): LanguageDetail
    ...
    type IDidFileOpenFn = (fileNode: IFileNode) => void;

    export function onDidFileOpen (fn: IDidFileOpenFn): IDisposable;
    /**
     * 具体文件及文件扩展名的图标映射
     * 例如指定
     *
  ...
  export namespace command {
    ...
    *      <div>
     *          <button onClick={() => executeCommand('Git.Commit', 'first commit')}>
     *              commit
     *          </button>
     *      </div>
     *  }
     * ```
     */
    export function executeCommand(commandType: string, payload?: any): void;
  ...
  export namespace actions {
    ...
    export function setStatusBarMessage(
      text: string,
      hideAfterTimeout: number | Promise<any>
    ): IDisposable;
  ...
  type IAppRegisterCallBackFn = (...args: Array<any>) => any;

  type IAppUnRegisterCallBackFn = IAppRegisterCallBackFn;

  type IPluginRegisterFn = (children: IPluginChildren, callback: IAppRegisterCallBackFn) => void;
   ...
   * 卸载插件
   */
  type IPluginUnRegisterFn = (children: any, callback?: IAppUnRegisterCallBackFn) => void;
  ...
  interface IRequest {
    ...
    postJSON: (options: any) => Promise<any>;
  }
  ...
  export interface II18nConf {
    customLanguagePool: ILanguagePool;
  }

  interface IStylusObject {
    use: () => void;
    unuse: () => void;
  }
  interface IStyles {
  ...
  interface IPluginContextConf {
    i18n?: II18nConf;
    styles?: IStyles;
    [propName: string]: any;
  }

  interface II18nScope {
    get: any;
  }
  ...
  export class PluginContext {
    constructor(config: IPluginContextConf);
    inializeData: any;
  ...
  interface IPluginConfiguration {
    ...
    title: string;
    /**
     * 注册的设置项，包含设置项的唯一 key、title、默认值、类型及描述等信息
     */
    properties: IPluginProperties;
  }

  type IPluginConfigChangeHandler = (propKey: string, oldValue: any, newValue: any) => void;
  ...
  interface IAdvanceProviderConnect {
    Provider: any
    connect: any
  }
  export function createAdvancedProviderAndConnect(storeKey: string): IAdvanceProviderConnect
  /**
   * 生成插件唯一 pluginKey
   */
  export function generatePluginKey():string;
}
